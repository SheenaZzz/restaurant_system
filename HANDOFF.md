...
**中英切换**（`i18n.ts`）：右上角一键全站切换，选择存 `localStorage`。
没引库，**词表直接用中文原文当 key**（gettext msgid 的路子），
缺词回落中文 —— 漏翻最坏也是显示中文，不会露出 key。

> ⚠️ **菜名 / 分类 / 加料名 / 时区名不进词表**，它们是数据不是文案：
> 后端各带一列 `name_en` / `label_en`，老板在改价页自己就能改。
> 塞进前端词表的话，老板加一道菜就得等人改代码发版。
>
> ⚠️ 加新界面时**翻译的单位是整句**。别把一句话用 `<b>` 切成几段
> 再逐段 `tr()` —— 语法上拼不回去，会出现半中半英。要强调就整句进词表、
> 用 CSS 强调。
>
> 验收用 `frontend/src/` 之外的独立检查：在浏览器里逐页扫 `innerText`
> 的 CJK 残留。别拿写代码时用的规则当验收标准，它漏掉的正是它看不见的。
