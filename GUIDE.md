...
| Step | 内容 | 完成标准 |
|---|---|---|
| **0** | 环境与仓库 | `docker run hello-world` 成功；repo 有 initial commit |
| **1** | **Walking Skeleton** ⭐ | iPad 飞行模式点 10 次 → 恢复 → 库里正好 10 条；重放两次仍是 10 条 |
| **2** | 数据模型落地（Alembic + 全部表 + 种子数据） | `\dt` 看到全部表；20 张桌 + 菜单可查 |
| **3** | 认证与 RBAC | `front` 账号调 admin 端点返回 403 |
| **4** | 开桌流程（第一个真实业务） | **掐表 5 秒内完成一次开桌** |
| **5** | 补菜记录 | 单手可点，3 秒完成 |
